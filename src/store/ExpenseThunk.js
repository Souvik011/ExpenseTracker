    ...
    const getExpenseFetching = async (email) => {
      try {
        const response = await fetch(
          `https://expense-tracker-5ef39-default-rtdb.firebaseio.com/expenses/${email}.json`,
          {
            method: "GET",
            headers: {
...
export const editExpenseFetching = (expenseItem, email) => {
  return async (dispatch) => {
    const editExpenseFetching = async (expenseItem, email) => {
      try {
        const response = await fetch(
          `https://expense-tracker-5ef39-default-rtdb.firebaseio.com/expenses/${email}/${expenseItem.id}.json`,
          {
            method: "PUT",
            body: JSON.stringify({
...
export const deleteExpenseFetching = (id, email) => {
  return async (dispatch) => {
    const deleteExpenseFetching = async (id, email) => {
      try {
        const response = await fetch(
          `https://expense-tracker-5ef39-default-rtdb.firebaseio.com/expenses/${email}/${id}.json`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
